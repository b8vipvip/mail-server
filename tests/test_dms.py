...
def test_rejects_bad_email():
    c=DMSClient()
    try: c.add_account("bad;rm -rf /","123456789012")
    except ValueError: pass
    else: raise AssertionError("invalid email accepted")
...
def test_uses_argv_not_shell(run):
    ...
    assert args == ["docker","exec","mailserver","setup","alias","add","a@example.com","b@example.com"]
